...
purrr::imap(scLANE_single_subj_lung_list, 
  ...
  tibble(
    ...
    N_PRED_DYNAMIC_GENES = sum(x$testDynamic_results_tidy[[1]]$Gene_Dynamic_Overall), 
...
purrr::imap(tradeSeq_single_subj_lung_list, 
  ...
  tibble(
    ...
    N_PRED_DYNAMIC_GENES = sum(x$tradeSeq_results_tidy[[1]]$gene_dynamic_overall), 
    RUNTIME = as.numeric(x$time_diff[[1]]), 
    RUNTIME_UNITS = attributes(x$time_diff[[1]])$units, 
    MEM_USED = x$mem_usage[[1]][1] / 1000^2,  # convert to megabytes
    MEM_UNITS = "MB", 
...
rm(scLANE_single_subj_lung_list, 
   scLANE_single_subj_panc_list, 
   tradeSeq_single_subj_lung_list, 
   tradeSeq_single_subj_panc_list)
  ...
  tibble(
    ...
    SPECIFICITY = yardstick::spec(x$testDynamic_results_tidy[[1]], 
                                  ...
                                  estimate = factor(ifelse(Gene_Dynamic_Overall == 1, "Dynamic", "NotDynamic"), 
                                                    levels = c("Dynamic", "NotDynamic"))) %>% 
                  pull(.estimate), 
    NIR = mean(x$testDynamic_results_tidy[[1]]$geneStatus_overall == "NotDynamic"), 
    ROC_CURVE = list(yardstick::roc_curve(x$testDynamic_results_tidy[[1]], 
                                          truth = factor(geneStatus_overall, levels = c("Dynamic", "NotDynamic")), 
                                          .estimate = P_Val, 
                                          event_level = "second") %>% 
...
# 250 cells
tar_load(tradeSeq_res_lung_DEG_10_CELLS_250_balanced)
tar_load(tradeSeq_res_lung_DEG_10_CELLS_250_unbalanced)
tar_load(tradeSeq_res_lung_DEG_20_CELLS_250_balanced)
tar_load(tradeSeq_res_lung_DEG_20_CELLS_250_unbalanced)
# 1,000 cells
...
# coerce to list 
tradeSeq_multi_subj_lung_list <- purrr::map(ls(pattern = "tradeSeq_res*"), \(x) eval(as.symbol(x)))
names(tradeSeq_multi_subj_lung_list) <- ls(pattern = "tradeSeq_res*")
rm(list = ls(pattern = "tradeSeq_res*")); gc(verbose = FALSE, full = TRUE)
# coerce to tibble
purrr::imap(tradeSeq_multi_subj_lung_list, 
            function(x, y) {
  tibble(
    DATASET_NAME = y, 
    MODEL_TYPE = "tradeSeq GAM", 
  ...
  mutate(RUNTIME_HOURS = case_when(RUNTIME_UNITS == "seconds" ~ RUNTIME / 60^2, 
                                   ...
                                   TRUE ~ NA_real_), 
         .before = 8)  %>% 
  mutate(DATASET_TYPE = "Multiple subjects", .before = 2) -> tradeSeq_multi_subj_lung_metrics
```
...
```{r, results='hide'}
# 100 cells
tar_load(tradeSeq_res_panc_DEG_10_CELLS_100_balanced)
tar_load(tradeSeq_res_panc_DEG_10_CELLS_100_unbalanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_100_balanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_100_unbalanced)
# 250 cells
tar_load(tradeSeq_res_panc_DEG_10_CELLS_250_balanced)
tar_load(tradeSeq_res_panc_DEG_10_CELLS_250_unbalanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_250_balanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_250_unbalanced)
# 1,000 cells
tar_load(tradeSeq_res_panc_DEG_10_CELLS_1000_balanced)
tar_load(tradeSeq_res_panc_DEG_10_CELLS_1000_unbalanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_1000_balanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_1000_unbalanced)
# 2,500 cells
tar_load(tradeSeq_res_panc_DEG_10_CELLS_2500_balanced)
tar_load(tradeSeq_res_panc_DEG_10_CELLS_2500_unbalanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_2500_balanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_2500_unbalanced)
# 5,000 cells
tar_load(tradeSeq_res_panc_DEG_10_CELLS_5000_balanced)
tar_load(tradeSeq_res_panc_DEG_10_CELLS_5000_unbalanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_5000_balanced)
tar_load(tradeSeq_res_panc_DEG_20_CELLS_5000_unbalanced)
# coerce to list 
tradeSeq_multi_subj_panc_list <- purrr::map(ls(pattern = "tradeSeq_res*"), \(x) eval(as.symbol(x)))
names(tradeSeq_multi_subj_panc_list) <- ls(pattern = "tradeSeq_res*")
rm(list = ls(pattern = "tradeSeq_res*")); gc(verbose = FALSE, full = TRUE)
# coerce to tibble
purrr::imap(tradeSeq_multi_subj_panc_list, 
            function(x, y) {
  tibble(
    DATASET_NAME = y, 
    MODEL_TYPE = "tradeSeq GAM", 
  ...
  mutate(RUNTIME_HOURS = case_when(RUNTIME_UNITS == "seconds" ~ RUNTIME / 60^2, 
                                   ...
                                   TRUE ~ NA_real_), 
         .before = 8)  %>% 
  mutate(DATASET_TYPE = "Multiple subjects", .before = 2) -> tradeSeq_multi_subj_panc_metrics
```
...
```{r}
# 100 cells
tar_load(Lamian_res_lung_DEG_10_CELLS_100_balanced)
tar_load(Lamian_res_lung_DEG_10_CELLS_100_unbalanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_100_balanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_100_unbalanced)
# 250 cells
tar_load(Lamian_res_lung_DEG_10_CELLS_250_balanced)
tar_load(Lamian_res_lung_DEG_10_CELLS_250_unbalanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_250_balanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_250_unbalanced)
# 1,000 cells
tar_load(Lamian_res_lung_DEG_10_CELLS_1000_balanced)
tar_load(Lamian_res_lung_DEG_10_CELLS_1000_unbalanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_1000_balanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_1000_unbalanced)
# 2,500 cells
tar_load(Lamian_res_lung_DEG_10_CELLS_2500_balanced)
tar_load(Lamian_res_lung_DEG_10_CELLS_2500_unbalanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_2500_balanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_2500_unbalanced)
# 5,000 cells
tar_load(Lamian_res_lung_DEG_10_CELLS_5000_balanced)
tar_load(Lamian_res_lung_DEG_10_CELLS_5000_unbalanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_5000_balanced)
tar_load(Lamian_res_lung_DEG_20_CELLS_5000_unbalanced)
# coerce to list 
Lamian_multi_subj_lung_list <- purrr::map(ls(pattern = "Lamian_res*"), \(x) eval(as.symbol(x)))
names(Lamian_multi_subj_lung_list) <- ls(pattern = "Lamian_res*")
rm(list = ls(pattern = "Lamian_res*")); gc(verbose = FALSE, full = TRUE)
# coerce to tibble
purrr::imap(Lamian_multi_subj_lung_list, 
            function(x, y) {
    tibble(
      DATASET_NAME = y, 
      MODEL_TYPE = "Lamian", 
      PERCENT_DEG = as.numeric(stringr::str_match(y, "_DEG_[0-9]*") %>% stringr::str_remove("_DEG_")) / 100, 
      N_CELLS = as.numeric(stringr::str_match(y, "_CELLS_[0-9]*") %>% stringr::str_remove("_CELLS_")), 
      N_GENES = nrow(x$Lamian_results_tidy[[1]]), 
      N_PRED_DYNAMIC_GENES = sum(x$Lamian_results_tidy[[1]]$gene_dynamic_overall),
      RUNTIME = as.numeric(x$time_diff[[1]]), 
      RUNTIME_UNITS = attributes(x$time_diff[[1]])$units, 
      MEM_USED = x$mem_usage[[1]][1] / 1000^2,  # convert to megabytes
      MEM_UNITS = "MB", 
      ROC_AUC = yardstick::roc_auc(x$Lamian_results_tidy[[1]], 
                                   truth = factor(geneStatus_overall, levels = c("Dynamic", "NotDynamic")), 
                                   .estimate = pvalue, 
                                   event_level = "second") %>% 
        pull(.estimate), 
      ACCURACY = yardstick::accuracy(x$Lamian_results_tidy[[1]], 
                                     truth = factor(geneStatus_overall, levels = c("Dynamic", "NotDynamic")), 
                                     estimate = factor(ifelse(gene_dynamic_overall == 1, "Dynamic", "NotDynamic"), 
                                                       levels = c("Dynamic", "NotDynamic"))) %>% 
        pull(.estimate), 
      SENSITIVITY = yardstick::sens(x$Lamian_results_tidy[[1]], 
                                    truth = factor(geneStatus_overall, levels = c("Dynamic", "NotDynamic")), 
                                    estimate = factor(ifelse(gene_dynamic_overall == 1, "Dynamic", "NotDynamic"), 
                                                      levels = c("Dynamic", "NotDynamic"))) %>% 
        pull(.estimate), 
      SPECIFICITY = yardstick::spec(x$Lamian_results_tidy[[1]], 
                                    truth = factor(geneStatus_overall, levels = c("Dynamic", "NotDynamic")), 
                                    estimate = factor(ifelse(gene_dynamic_overall == 1, "Dynamic", "NotDynamic"), 
                                                      levels = c("Dynamic", "NotDynamic"))) %>% 
        pull(.estimate), 
      NIR = mean(x$Lamian_results_tidy[[1]]$geneStatus_overall == "NotDynamic"), 
      ROC_CURVE = list(yardstick::roc_curve(x$Lamian_results_tidy[[1]], 
                                            truth = factor(geneStatus_overall, levels = c("Dynamic", "NotDynamic")), 
                                            .estimate = pvalue, 
                                            event_level = "second") %>% 
                         mutate(dataset = y))
  ...
  mutate(RUNTIME_HOURS = case_when(RUNTIME_UNITS == "seconds" ~ RUNTIME / 60^2, 
                                   ...
                                   TRUE ~ NA_real_), 
         .before = 8)  %>% 
  mutate(DATASET_TYPE = "Multiple subjects", .before = 2) -> Lamian_multi_subj_lung_metrics
```
...
```{r}
# 100 cells
tar_load(Lamian_res_panc_DEG_10_CELLS_100_balanced)
tar_load(Lamian_res_panc_DEG_10_CELLS_100_unbalanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_100_balanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_100_unbalanced)
# 250 cells
tar_load(Lamian_res_panc_DEG_10_CELLS_250_balanced)
tar_load(Lamian_res_panc_DEG_10_CELLS_250_unbalanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_250_balanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_250_unbalanced)
# 1,000 cells
tar_load(Lamian_res_panc_DEG_10_CELLS_1000_balanced)
tar_load(Lamian_res_panc_DEG_10_CELLS_1000_unbalanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_1000_balanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_1000_unbalanced)
# 2,500 cells
tar_load(Lamian_res_panc_DEG_10_CELLS_2500_balanced)
tar_load(Lamian_res_panc_DEG_10_CELLS_2500_unbalanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_2500_balanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_2500_unbalanced)
# 5,000 cells
tar_load(Lamian_res_panc_DEG_10_CELLS_5000_balanced)
tar_load(Lamian_res_panc_DEG_10_CELLS_5000_unbalanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_5000_balanced)
tar_load(Lamian_res_panc_DEG_20_CELLS_5000_unbalanced)
# coerce to list 
Lamian_multi_subj_panc_list <- purrr::map(ls(pattern = "Lamian_res*"), \(x) eval(as.symbol(x)))
names(Lamian_multi_subj_panc_list) <- ls(pattern = "Lamian_res*")
rm(list = ls(pattern = "Lamian_res*")); gc(verbose = FALSE, full = TRUE)
# coerce to tibble
purrr::imap(Lamian_multi_subj_panc_list, 
            function(x, y) {
  tibble(
    DATASET_NAME = y, 
    MODEL_TYPE = "Lamian", 
  ...
  mutate(RUNTIME_HOURS = case_when(RUNTIME_UNITS == "seconds" ~ RUNTIME / 60^2, 
                                   ...
                                   TRUE ~ NA_real_), 
         .before = 8)  %>% 
  mutate(DATASET_TYPE = "Multiple subjects", .before = 2) -> Lamian_multi_subj_panc_metrics
```


```{r, echo=FALSE, results='hide', eval=TRUE}
# cleanup
rm(scLANE_multi_subj_lung_GEE_list, 
   scLANE_multi_subj_panc_GEE_list, 
   scLANE_multiple_subj_lung_even_GLMM_list, 
   scLANE_multiple_subj_panc_even_GLMM_list, 
   scLANE_multiple_subj_lung_adaptive_GLMM_list, 
   scLANE_multiple_subj_panc_adaptive_GLMM_list, 
   tradeSeq_multi_subj_lung_list, 
   tradeSeq_multi_subj_panc_list)
gc(verbose = FALSE, full = TRUE)
```

Lastly, we combine all all the individual metric table into a single overall table and save it as a compressed `.Rds` file. 
...
```{r, fig.width=5}
p <- metric_table %>% 
     filter(stringr::str_detect(DATASET_NAME, "Lamian_")) %>% 
     pull(ROC_CURVE) %>% 
     purrr::reduce(rbind) %>% 
     mutate(P_DEG = paste0(as.numeric(substr(stringr::str_extract(dataset, "DEG_[0-9][0-9]"), 5, 6)), "%"), 
            N_CELLS = stringr::str_replace(stringr::str_extract(dataset, "CELLS_.*"), "CELLS_", ""), 
            N_CELLS = stringr::str_replace(N_CELLS, "_balanced", ""), 
            P_DEG = factor(P_DEG, levels = c("1%", "5%", "10%", "20%")), 
            N_CELLS = factor(N_CELLS, levels = as.character(sort(as.numeric(unique(N_CELLS))))), 
            REF = case_when(grepl("panc", dataset) ~ "Pancreas", 
                            TRUE ~ "Lung")) %>% 
     ggplot(aes(x = 1 - specificity, y = sensitivity, group = dataset, color = P_DEG)) + 
     # facet_wrap(~P_DEG) + 
     geom_line(size = 1, alpha = 0.8) + 
     geom_abline(slope = 1, intercept = 0, color = "black", linetype = "dashed", size = 1) + 
     scale_x_continuous(labels = scales::percent_format(accuracy = 1)) + 
     ...
     labs(x = "1 - Specificity", 
          ...
          title = "ROC-AUC", 
          color = "Number of Cells", 
          subtitle = "Lamian on Multi-subject Simulated Data") + 
     theme_classic(base_size = 15) + 
     theme(plot.title = element_text(hjust = 0.5), 
           plot.subtitle = element_text(hjust = 0.5, face = "italic", size = 13), 
           plot.title.position = "panel")
