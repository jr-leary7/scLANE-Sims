...
# 2,500 cells
tar_load(scLANE_res_DEG_01_CELLS_2500)
tar_load(scLANE_res_DEG_05_CELLS_2500)
tar_load(scLANE_res_DEG_10_CELLS_2500)
tar_load(scLANE_res_DEG_20_CELLS_2500)
# 5,000 cells
...
# 2,500 cells
tar_load(tradeSeq_res_DEG_01_CELLS_2500)
tar_load(tradeSeq_res_DEG_05_CELLS_2500)
tar_load(tradeSeq_res_DEG_10_CELLS_2500)
tar_load(tradeSeq_res_DEG_20_CELLS_2500)
# 5,000 cells
...
# coerce to list 
tradeSeq_single_subj_list <- purrr::map(ls(pattern = "tradeSeq_res*"), \(x) { eval(as.symbol(x)) })
...
rm(scLANE_single_subj_list, tradeSeq_single_subj_list); gc(verbose = FALSE, full = TRUE)
```
...
# 2,500 cells
tar_load(scLANE_res_DEG_10_CELLS_2500_balanced)
tar_load(scLANE_res_DEG_10_CELLS_2500_unbalanced)
tar_load(scLANE_res_DEG_20_CELLS_2500_balanced)
tar_load(scLANE_res_DEG_20_CELLS_2500_unbalanced)
# 5,000 cells
...
```{r, echo=FALSE, results='hide'}
rm(c(scLANE_multi_subj_list, tradeSeq_multi_subj_list)); gc(verbose = FALSE, full = TRUE)
```
