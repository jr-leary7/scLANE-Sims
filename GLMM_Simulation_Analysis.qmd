...
```{r}
x = scLANE_GLMM_adaptive_list[[8]] 
y = names(scLANE_GLMM_adaptive_list)[8]
a = try({
      tibble(
        ...
        N_DYNAMIC_GENES = sum(x$testDynamic_results_tidy[[1]]$geneStatus_overall == "Dynamic"), 
        N_NONDYNAMIC_GENES = length(unique(names(x$testDynamic_results_raw[[1]]))) - sum(x$testDynamic_results_tidy[[1]]$geneStatus_overall == "Dynamic"), 
        N_PRED_DYNAMIC_GENES = sum(x$testDynamic_results_tidy[[1]]$Gene_Dynamic_Overall == 1), 
        N_PRED_NONDYNAMIC_GENES = sum(x$testDynamic_results_tidy[[1]]$Gene_Dynamic_Overall == 0), 
        N_MARGE_MODEL_ERRORS = sum(grepl("marge model error", tolower(x$testDynamic_results_tidy[[1]]$Model_Status))), 
...
purrr::imap(scLANE_GLMM_adaptive_list, 
  ...
  mutate(RUNTIME_HOURS = case_when(RUNTIME_UNITS == "secs" ~ RUNTIME / 60^2, 
                                   ...
                                   TRUE ~ NA_real_), 
         .before = which(colnames(.) == "MEM_USED")) %>% 
